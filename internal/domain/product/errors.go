...
var (
	...
	ErrExecutionModelLocked   = shared.New("EXECUTION_MODEL_LOCKED", "Execution model cannot be changed after planning work exists", 409)
	ErrInvalidCustomLevels    = shared.New("INVALID_CUSTOM_LEVELS", "Custom model needs 1–3 unique non-empty level labels", 400)
	...
	ErrInvalidProductMemberRole = shared.New("INVALID_PRODUCT_MEMBER_ROLE", "Invalid product member role", 400)
	ErrProductManagerRequired = shared.New("PRODUCT_MANAGER_REQUIRED", "Product manager is required", 400)
	ErrProductOnHold          = shared.New("PRODUCT_ON_HOLD", "Product is on hold — resume it before moving the workflow", 409)
	ErrProductNameTaken       = shared.New("PRODUCT_NAME_TAKEN", "A product with this name already exists", 409)
	ErrProductCodeTaken       = shared.New("PRODUCT_CODE_TAKEN", "Product code already in use", 409)
)
