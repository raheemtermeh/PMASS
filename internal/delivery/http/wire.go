...
func NewDependencies(sqlDB *sql.DB) *Dependencies {
	...
	prodMembers := postgres.NewProductMemberRepo(db)
	...
	checklist := postgres.NewChecklistRepo(db)
	featureDeps := postgres.NewFeatureDependencyRepo(db)
	projMembers := postgres.NewProjectMemberRepo(db)
	featMembers := postgres.NewFeatureMemberRepo(db)

	orgSvc := organizationapp.NewService(db, co, dept, team, emp)
	prodSvc := productapp.NewService(db, prod, pipe, stage, si, prodMembers, emp, act, notif)
	planSvc := planningapp.NewService(db, proj, feat, task, prod, act, notif, checklist, featureDeps, projMembers, featMembers, emp)
